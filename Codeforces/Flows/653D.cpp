// 653D from codeforces solved using Dinic algorithm
#include <cstdint>
#include <iostream>
#include <vector>
#include <queue>
...
{ 
    ...
    const_adj = vector<vector<int>>(n+1);
    for(int i = 0; i < m; i++)
    {
        int x, y, c;
        cin >> x >> y >> c;
        if(const_graph[x][y] == 0 && const_graph[y][x] == 0)
        {
            const_adj[x].push_back(y);
            const_adj[y].push_back(x);
        }
        if(x == 1)
            max_carry += c;
        const_graph[x][y] += c;
    ...
    max_carry = max_carry / x + 1;

    while(max_carry*x- min_carry*x > diff)

    {

        double curr = (max_carry + min_carry) / 2;
        ...
        for(int i = 1; i <= n; i++)
        {
            for(auto it : const_adj[i])
            {
                unsigned long long v = const_graph[i][it] / curr;
                

                if(graph[i][it] == 0 && graph[it][i] == 0 && v > 0)
                {
                    adj[i].push_back(it);
                    adj[it].push_back(i);
                }
